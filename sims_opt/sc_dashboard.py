...
def get_pies(filtered_df):
    ...
    #print(filtered_df)
    if filtered_df.shape[0] > 0:
        pie_demand = filtered_df[['wr_id', 'demand']].groupby('wr_id').sum().reset_index()
    else:
...
def get_waterfall(filtered_df, trp_cost, op_cost):
    ...
    if filtered_df.shape[0] > 0:
        bar_pl = pd.DataFrame(filtered_df[['supply', 'demand']].sum(axis=0)).reset_index()
        bar_pl.columns = ['supply/demand', 'value']
    else:
